  ...
  return (
    <Slide className="slide-light" style={{ background: "var(--color-surface-cream)", color: "var(--color-slate)" }}>
      ...
      <AnimateIn delay={0.3}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.4rem", marginBottom: "0.4625rem" }}>
          {glanceFacts.map((fact, i) => (
            <div key={fact.label} style={{ background: "var(--color-slate)", borderRadius: 5, padding: "0.4rem 0.5rem", display: "flex", alignItems: "center", gap: "0.4rem" }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={i % 2 === 0 ? "var(--color-orange)" : "rgba(255,255,255,0.7)"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d={fact.icon} /></svg>
              <div>
                <div style={{ fontFamily: "var(--font-serif)", fontWeight: 700, fontSize: "0.78rem", color: "white", lineHeight: 1.1 }}>{fact.value}</div>
                <div style={{ fontFamily: "Inter, system-ui, sans-serif", fontSize: "0.44rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: "rgba(255,255,255,0.55)", marginTop: "0.0625rem" }}>{fact.label}</div>
              </div>
            </div>
          ))}
        </div>
